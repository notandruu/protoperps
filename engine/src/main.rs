...
impl MarketState {
    ...
    fn ensure_margin(&mut self, trader: &str) -> &mut MarginAccount {
        self.margins
            ...
            .or_insert_with(|| MarginAccount::new(trader))
    }
...
async fn main() -> Result<()> {
    ...
    let states: Arc<HashMap<String, Arc<RwLock<MarketState>>>> = Arc::new(
        cfg.markets
            .iter()
            .map(|s| (s.clone(), Arc::new(RwLock::new(MarketState::new(s)))))
            .collect(),
    );
    ...
    loop {
        ...
        let cmd: OrderCommand = match serde_json::from_slice(payload) {
            ...
            Err(e) => {
                ...
                producer
                    .send_or_dlq(ORDERS_IN, DLQ_ORDERS, "parse_error", &String::from_utf8_lossy(payload).to_string())
                    .await;
                continue;
            }
        };
        ...
        match cmd {
            OrderCommand::Place(params) => {
                ...
                let margin = st.margins.get_mut(&trader).unwrap();
                let position = st.positions.get_mut(&trader).unwrap();
                let mut maker_positions: HashMap<String, Position> = HashMap::new();

                // Collect maker positions for fills we're about to compute
                // (simplified: real impl would pre-load makers from the book snapshot)

                let result = place_order(&params, &mut st.market, margin, position, &mut maker_positions);
                ...
                match result {
                    ...
                    Err(e) => {
                        warn!("order rejected: {e}; routing to DLQ");
                        producer
                            .send_or_dlq(ORDERS_IN, DLQ_ORDERS, &symbol, &params)
                            .await;
                    }
                }
            }
